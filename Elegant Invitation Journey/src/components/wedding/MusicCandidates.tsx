 ...
 * never see it. Each entry is switched to immediately rather than faded, since
 * the point is comparing them; any file that will not load is marked rather
 * than failing silently.
 *
 * Delete this file and its use in Microsite once a track is chosen.
 */
const CANDIDATES = [
  { file: "/music/invitation.m4a", label: "Current" },
  { file: "/music/candidates/1.m4a", label: "One" },
  { file: "/music/candidates/2.m4a", label: "Two" },
  { file: "/music/candidates/3.m4a", label: "Three" },
  { file: "/music/candidates/4.m4a", label: "Four" },
  { file: "/music/candidates/5.m4a", label: "Five" },
];
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {CANDIDATES.map((c) => {
          ...
          return (
            <button
              key={c.file}
              type="button"
              // Pointer-down, so the first tap takes — see MusicToggle.
              onPointerDown={() => choose(c.file)}
              onClick={(e) => {
                if (e.detail === 0) choose(c.file);
              }}
              aria-pressed={on}
        ...
        <button
          type="button"
          onPointerDown={stopMusic}
          onClick={(e) => {
            if (e.detail === 0) stopMusic();
          }}
